...
print("ViT-B/16-specific imports completed.")
...
# It is written in notes/config through EXPERIMENT_NOTES.
MODEL_VARIANT = "vit_b16_torchvision"

# General training settings
EPOCHS = 5
BATCH_SIZE = 16                            # ViT is heavier than EfficientNet/ConvNeXt, so 16 is safer
LEARNING_RATE = 5e-5                       # ViT usually works better with a smaller LR
...
EXPERIMENT_NOTES = "ViT-B/16 torchvision baseline with basic augmentation"

print("Shared parameters loaded.")
print(f"Runner: {RUNNER_NAME}")
print(f"Model folder/name: {MODEL_NAME}")
print(f"Model variant: {MODEL_VARIANT}")
print(f"Run type: {RUN_TYPE}")

# ============================================================
# 16. VIT-B/16 EXTRA PARAMETERS
# ============================================================
...
GRAD_CLIP_NORM = 1.0             # ViT can benefit from gradient clipping
...
print("ViT-B/16 extra parameters loaded.")
...
# 18. IMAGE TRANSFORMS
# ============================================================
# These transforms prepare images for ViT-B/16.
# Torchvision ViT pretrained weights expect ImageNet-style normalization.

# ImageNet mean and std are used because ViT-B/16 was pretrained on ImageNet.
...
# ============================================================
# 21. CREATE VIT-B/16 MODEL
# ============================================================
# We use pretrained ViT-B/16 and replace the final classification head for binary classification.

def create_vit_b16(pretrained=True, freeze_backbone=False):
    """
    Create ViT-B/16 for binary real vs synthetic image classification.

    Parameters:
        pretrained (bool): Whether to load ImageNet pretrained weights.
        freeze_backbone (bool): Whether to freeze transformer backbone layers.

    Returns:
        torch.nn.Module: ViT-B/16 model.
    """
    ...
    # In torchvision ViT, the main feature extractor is stored in encoder + conv_proj.
    if freeze_backbone:
    ...
    # Torchvision ViT stores its classification head inside model.heads.
    # Usually:
    # Sequential(
    #   (head): Linear(in_features=768, out_features=1000)
    # )
    in_features = model.heads.head.in_features

    # Replace final head with one output logit.
